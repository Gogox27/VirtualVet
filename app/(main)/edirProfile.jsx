...
import Camara from "../../assets/icons/Camara";
import Input from "../../components/Input";
import Person from "../../assets/icons/Person";
import Celular from "../../assets/icons/Celular";
...
const EditarPerfil = () => {
  ...
  const onSubmit = async () => {
    ...
    if (!name || !phoneNumber || !address || !bio || !image) {
      Alert.alert("Perfil", "Porfavor llena todos los campos");
      return;
    }
    setLoading(true);
  ...
  let onPickImage = async () => {
    ...
    if (!result.canceled && result.assets.length > 0) {
      setUser({ ...user, image: result.assets[0] }); // antes era result.assets[0].uri
    }
  };
  let imageSource =
        ...
        <ScrollView style={{ flex: 1 }}>
          <View style={styles.form}>
            ...
            <Text style={{ fontSize: hp(1.5), color: theme.colors.text }}>
              Porfavor llena tu perfil detalladamente
            </Text>
            <Input
              icon={<Person></Person>}
